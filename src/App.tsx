...
import { NavRail, TabType } from './components/layout/NavRail';
...
import { MediaPlayerModal } from './components/files/MediaPlayerModal';
...
export const App: React.FC = () => {
  ...
  const [userRole, setUserRole] = useState<string>(() => {
    try {
      const u = JSON.parse(localStorage.getItem('pocketcloud_user') || '{}');
      return u.role || 'owner';
    } catch (e) {
      return 'owner';
    }
  ...
  // Modals for Media Preview and Share
  const [mediaModalItem, setMediaModalItem] = useState<FileItem | null>(null);
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      <Box sx={{ display: 'flex', flex: 1, flexDirection: { xs: 'column', md: 'row' } }}>
        ...
        <Box sx={{ flex: 1, p: { xs: 2, md: 4 }, overflowX: 'hidden' }}>
            ...
            <>
              ...
              {loading ? (
                ...
                <FileTable
                  ...
                  onOpenMedia={(item) => setMediaModalItem(item)}
          ...
          {activeTab === 'system' && (
            <SystemDashboard onShowNotification={showNotification} />
          )}

          {activeTab === 'users' && (
            <UsersTab onShowNotification={showNotification} />
          )}
